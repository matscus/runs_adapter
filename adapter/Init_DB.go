...
var (
	...
	CREATE TABLE IF NOT EXISTS tests.tReleases (id UUID UNIQUE NOT NULL, name VARCHAR(45), release_date_time TIMESTAMP,project_id UUID REFERENCES tests.tProjects(id) ON DELETE CASCADE, PRIMARY KEY (id));
	...
	INSERT INTO tests.treleases(id, "name", release_date_time, project_id)VALUES(uuid_generate_v4(), 'global', now(), (select id from tests.tProjects where space_id=(select id from tests.tSpaces where name='detmir')));
	...
	INSERT INTO tests.tLoadTypes (id,name)values(uuid_generate_v4(),'lenear') ON CONFLICT DO NOTHING;
